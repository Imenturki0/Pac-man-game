...
class Ghost:
    ...
    def draw(self, screen, powerup, eaten_ghost):
        ...
        print("eaten",eaten_ghost,"dead",self.dead)
        ...
        if not powerup and (not eaten_ghost[self.id] or not self.dead) or (self.in_box and eaten_ghost[self.id]) :
            screen.blit(self.img, (self.x_pos, self.y_pos))
        ...
        #screen.blit(dead_img, (self.x_pos, self.y_pos))
        print( eaten_ghost[self.id])
        pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(self.num_width*9, self.num_height*8, self.num_width, self.num_height*3), 2)
